 ...
 if(carrier_ext_seen) begin
      ...
      crc_ok = frame_unpack(
	   ...
	   if(tr.vlan_en) begin
          ...
          //--------------------------------------
          // Reserved VLAN ID check
          //--------------------------------------
          if(tr.VID == 12'hFFF) begin
 		bad_pkt=1;
           // globals::v_uif[mac_addr].tx_bad_pkt_count++;
            `uvm_warning("VLAN_RESERVED_VID",
              $sformatf(
                "Reserved VLAN ID detected VID=%h",
                tr.VID))
             // continue;
          end
          ...
          if(tr.VID == 12'h000) begin
            ...
            `uvm_info("VLAN_PRIORITY_TAG",
              "Priority tagged VLAN frame detected",
              UVM_LOW)
...
if(!crc_ok)
  pkt_bad = 1;
      ...
      // PAUSE
      //------------------------------------------------
      if(tr.pause_frame_en && tr.ether_type == 16'h8808 && tr.pause_opc==16'h0001)
...
if(tr.payload.size() < 46 &&
  ...
  //  - runt
  //  - good packet
  //  - unicast/multicast/broadcast already handled
  //--------------------------------------------
  if(crc_ok) begin
  ...
  //--------------------------------------------
    ...
    `uvm_error("TX_FRAGMENT",
      $sformatf(
        "Fragment detected payload=%0d",
        tr.payload.size()))
...
if(len_mismatch_rx &&
  ...
  `uvm_error("RX_LEN_MISMATCH",
    $sformatf(
      "Length mismatch DA=%h SA=%h payload=%0d",
      tr.da,
      tr.sa,
      tr.payload.size()))
   ...
   globals::v_uif[mac_addr].rx_broadcast_count++;
	 ...
	 //---------------------------------------------------- 
      ...
      //------------------------------------------------
       ...
       if(tr.pfc_frame_en && tr.ether_type == 16'h8808 && tr.pause_opc   == 16'h0005) begin

            //------------------------------------------
            // Store PFC pause information
            //------------------------------------------
            for(int i=0;i<8;i++) begin
