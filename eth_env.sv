class eth_env extends uvm_env;
  ...
  function void connect_phase(uvm_phase phase);
    ...
    foreach(agnt_mac[i]) begin
      ...
      agnt_mac[i].mon_h.tx_ap.connect(sbscr_h.tx_fifo[i].analysis_export);
      agnt_mac[i].mon_h.rx_ap.connect(sbscr_h.rx_fifo[i].analysis_export);       
    end
      
    
    for (int i = 0; i < `NO_OF_AGENTS; i++) begin
      vseqr_h.mac[i] = agnt_mac[i].seqr_h;  
